...
public let requestClosure = { (endpoint: Endpoint, done: MoyaProvider.RequestResultClosure) in
    do {
        ...
        if let requestData = request.httpBody {
            ...
            }else if endpoint.url.contains("login/mobileLogin") || endpoint.url.contains("login/passLogin") || endpoint.url.contains("captcha/getCaptchaCode") || endpoint.url.contains("category/getCategoryInfoList") || endpoint.url.contains("shop/forgetPass") || endpoint.url.contains("shop/changePass") || endpoint.url.contains("shop/regAccount") || endpoint.url.contains("prodmanage/getProductInfoList") || endpoint.url.contains("category/getProductCategoryList") || endpoint.url.contains("brand/getProductBrandList") || endpoint.url.contains("spec/getProductSpecList") || endpoint.url.contains("spu/getProductSpuList"){
                //手机号登录 密码登录 发送验证码 经营种类 忘记密码 修改密码 注册账号  获取商品列表 获取商品类目 获取品牌列表 获取商品规格列表  获取商品参数列表
